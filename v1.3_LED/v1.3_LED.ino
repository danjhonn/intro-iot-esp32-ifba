...
void setup() {        //O código aqui é executado apenas uma vez, ao iniciar a execução
  configuraLED();     //Chamada à função configuraLED, que realiza a configuração dos pinos e dos canais PWM de cada LED
}

void loop() {         //O código aqui é executado repetidamente
  controlaLED();      //Chamada à função controlaLED, que atualiza o estado do LED
}
...
void controlaLED(){         //Atualiza o estado do LED
  int r = 0, g = 0, b = 0;
